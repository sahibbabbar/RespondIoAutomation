...
public class CreateBookingTests extends CommonApiTest {
    ...
    public void createBookingTest() throws JsonProcessingException {
        ...
        Assert.assertEquals(response.getStatusCode(), 200, "Verify that the valid status code is returned.");
        Assert.assertNotNull(booking, "Verify that the response is not null");
        Assert.assertNotNull(booking.getBookingid(), "Verify that the booking id is not null");

        Assert.assertEquals(booking.getBooking().getFirstName(), bookingDto.getFirstName(), "Verify the firstname is correct.");
        Assert.assertEquals(booking.getBooking().getLastName(), bookingDto.getLastName(), "Verify the lastname is correct.");
        Assert.assertEquals(booking.getBooking().getTotalPrice(), bookingDto.getTotalPrice(), "Verify the total price is correct.");
        Assert.assertEquals(booking.getBooking().getDepositPaid(), bookingDto.getDepositPaid(), "Verify the deposit paid is correct.");
        Assert.assertEquals(booking.getBooking().getBookingDates().getCheckIn(), bookingDto.getBookingDates().getCheckIn(), "Verify the booking check-in date is correct.");
        Assert.assertEquals(booking.getBooking().getBookingDates().getCheckOut(), bookingDto.getBookingDates().getCheckOut(), "Verify the booking check-out date is correct.");
        Assert.assertEquals(booking.getBooking().getAdditionalNeeds(), bookingDto.getAdditionalNeeds(), "Verify the additional needs are correct.");
    }
    ...
    public void createBookingWithoutFirstNameTest() {
        ...
        Assert.assertEquals(response.getStatusCode(), 500, "Verify that the valid errors status code is returned.");
        Assert.assertEquals(response.asString(), "Internal Server Error", "Verify that the API responding valid error.");
    }
